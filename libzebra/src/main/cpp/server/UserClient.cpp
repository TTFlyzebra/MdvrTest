...
UserClient::UserClient(UserServer *server, Notify *notify, int32_t socket)
        : BaseNotify(notify), mServer(server), mSocket(socket), recv_t(nullptr), send_t(nullptr),
          hand_t(nullptr), fixed_t(nullptr) {
    {
...
void UserClient::handle(NofifyType type, const char *data, int32_t dsize, const char *params,
                        int32_t psize) {
    switch (type) {
    ...
    if (connected()) {
        ...
        while (!is_stop) {
            ...
            if (recvLen > 0) {
                ...
                FLOGD("UserClient exit. recvLen[%d], errno[%s(%d)].", recvLen, strerror(errno),
                      errno);
                break;
            }
        }
    ...
    while (!is_stop) {
        {
            ...
            if (((recvBuf[0] & 0xFF) != 0xEE) || ((recvBuf[1] & 0xFF) != 0xAA)) {
                FLOGE("UserClient handleData bad header[%02x:%02x]", recvBuf[0] & 0xFF,
                      recvBuf[1] & 0xFF);
                recvBuf.clear();
                continue;
            }
