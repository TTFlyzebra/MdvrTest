...
,a_rtcp_socket(-1)
,artcp_t(nullptr)
,sequencenumber1(1234)
,sequencenumber2(4321)
{
    FLOGD("%s()", __func__);
    FD_ZERO(&set);
    FD_SET(mSocket, &set);
...
void RtspClient::handle(int32_t type, const char* data, int32_t size, int32_t p1, int32_t p2, int32_t p3, int64_t p4, int64_t tid)
{
    switch (type) {
...
void RtspClient::recvThread()
{
    ...
    while(!is_stop){
        ...
        if (recvLen <= 0) {
            ...
            FLOGD("RtspClient disconnect, socket[%d]recvLen[%d][%s(%d)].", mSocket, recvLen, strerror(errno), errno);
            is_stop = true;
            break;
        }else{
            std::lock_guard<std::mutex> lock (mlock_recv);
...
void RtspClient::videoRtpThread()
{
    ...
    while(!is_stop){
        int32_t recvLen = recvfrom(socket, recvdata, 1024, 0, (struct sockaddr *)&v_rtp_addr_in, (socklen_t *)&v_rtp_addrLen);
        if(recvLen > 0){
            memset(temp,0, 4096);
            ...
            disconnect();
            FLOGD("RtspClient Video rtp recv exit! socket[%d]recvLen[%d][%s(%d)].", socket, recvLen, strerror(errno), errno);
            break;
        }
    }
}
...
void RtspClient::videoRtcpThread()
{
    ...
    while(!is_stop){
        int32_t recvLen = recvfrom(socket, recvdata, 1024, 0, (struct sockaddr *)&v_rtcp_addr_in, (socklen_t *)&v_rtcp_addrLen);
        if(recvLen > 0){
            memset(temp, 0, 4096);
            for (int32_t i = 0; i < recvLen; i++) {
                sprintf(temp, "%s%02x:", temp, recvBuf[i]);
            ...
            FLOGD("RtspClient Video rtcp recv exit! socket[%d]recvLen[%d][%s(%d)].", socket, recvLen, strerror(errno), errno);
            disconnect();
            break;
        }
    }
...
void RtspClient::audioRtpThread()
{
    ...
    while(!is_stop){
        int32_t recvLen = recvfrom(socket, recvdata, 1024, 0, (struct sockaddr *)&a_rtp_addr_in, (socklen_t *)&a_rtp_addrLen);
        if(recvLen > 0){
            memset(temp, 0, 4096);
            for (int32_t i = 0; i < recvLen; i++) {
                sprintf(temp, "%s%02x:", temp, recvBuf[i]);
            ...
            FLOGD("RtspClient Audio rtp recv exit! socket[%d]recvLen[%d][%s(%d)].", socket, recvLen, strerror(errno), errno);
            disconnect();
            break;
        }
    }
...
void RtspClient::audioRtcpThread()
{
    ...
    while(!is_stop){
        int32_t recvLen = recvfrom(socket, recvdata, 1024, 0, (struct sockaddr *)&a_rtcp_addr_in, (socklen_t *)&a_rtcp_addrLen);
        if(recvLen > 0){
            memset(temp, 0, 4096);
            for (int32_t i = 0; i < recvLen; i++) {
                sprintf(temp, "%s%02x:", temp, recvBuf[i]);
            ...
            FLOGD("RtspClient Audio rtcp recv exit! socket[%d]recvLen[%d][%s(%d)].", socket, recvLen, strerror(errno), errno);
            disconnect();
            break;
        }
    }
...
void RtspClient::disconnect()
{
    if(!is_disconnect){
        ...
        switch(mSiteLink){
        case CAMERA:
            N->miniNotify((const char*)CAMOUT_U_STOP, sizeof(CAMOUT_U_STOP), T->tid, (int64_t)mUid);
            break;
        case SCREEN:
        default:
            N->miniNotify((const char*)SCREEN_U_STOP, sizeof(SCREEN_U_STOP), T->tid, (int64_t)mUid);
            N->miniNotify((const char*)SNDOUT_U_STOP, sizeof(SNDOUT_U_STOP), T->tid, (int64_t)mUid);
            break;
        }
...
void RtspClient::onDescribeRequest(const char* data, int32_t cseq)
{
    ...
    switch(mSiteLink){
        ...
        {
            char data[8] = {0x00};
            ByteUtil::int16ToData(data, T->camera.w);
            N->miniNotify((const char*)CAMOUT_U_READY, sizeof(CAMOUT_U_READY), T->tid, (int64_t)mUid, data);
            vec_sps.insert(vec_sps.end(), sps_720x1280, sps_720x1280 + sizeof(sps_720x1280));
            vec_pps.insert(vec_pps.end(), pps_720x1280, pps_720x1280 + sizeof(pps_720x1280));
            break;
        }
        case SCREEN:
        default:
        {
            char data[8] = {0x00};
            ByteUtil::int16ToData(data, DEFAULT_DPI);
            N->miniNotify((const char*)SCREEN_U_READY, sizeof(SCREEN_U_READY), T->tid, (int64_t)mUid, data);
            N->miniNotify((const char*)SNDOUT_U_READY, sizeof(SNDOUT_U_READY), T->tid, (int64_t)mUid);
            vec_sps.insert(vec_sps.end(), sps_720x1440, sps_720x1440 + sizeof(sps_720x1440));
            vec_pps.insert(vec_pps.end(), pps_720x1440, pps_720x1440 + sizeof(pps_720x1440));
            break;
        }
    }
    ...
    if(vec_sps.empty() || vec_pps.empty()) {
        ...
        memset(temp, 0, strlen(temp));
        sprintf(temp, "a=fmtp:96 profile-level-id=1;packetization-mode=1;sprop-parameter-sets=%s,%s\r\n",sps,pps);
        spd.append(temp);
    }
...
void RtspClient::onSetupRequest(const char* data, int32_t cseq)
{
    ...
    if (strncmp(strstr((const char*)data, "RTP/AVP"), "RTP/AVP/TCP", 11) == 0) {
        ...
        if(strstr((const char*)data, "track1")){
            ...
            getpeername(mSocket, (struct sockaddr *)&v_rtcp_addr_in, (socklen_t*)&v_rtcp_addrLen);
        ...
        //audio
            ...
            getpeername(mSocket, (struct sockaddr *)&a_rtcp_addr_in, (socklen_t*)&a_rtcp_addrLen);
...
void RtspClient::sendVFrame(const char* avc, int32_t size, int64_t ptsUsec)
{
    ...
    if(size <= fau_num){
        ...
        if(is_use_tcp){
            ...
            sendto(v_rtp_socket, send_pack+4, 12+size, 0, (struct sockaddr*)&v_rtp_addr_in, v_rtp_addrLen);
        }
    } else {
        int32_t num = 0;
        while((size-1-num*fau_num)>0){
            ...
            if(is_use_tcp){
                ...
                sendto(v_rtp_socket, send_pack+4, 14+rtpsize, 0, (struct sockaddr*)&v_rtp_addr_in, v_rtp_addrLen);
            }
            num++;
        }
    }
...
void RtspClient::sendAFrame(const char* aac, int32_t size, int64_t ptsUsec)
{
    ...
    if(is_use_tcp){
        ...
        sendto(a_rtp_socket, send_pack+4, 16+size, 0, (struct sockaddr*)&a_rtp_addr_in, a_rtp_addrLen);
    }
}
...
void RtspClient::selfFixedThread()
{
    while(!is_stop){
        ...
        //send heart
        N->miniNotify((const char*)U_HEARTBEAT, sizeof(U_HEARTBEAT), T->tid, (int64_t)mUid, heart_data);
    }
}
